...
## Phase F1 � Frontend Architecture & Scaffolding
...
frontend/src/
  ...
  components/   # Reusable UI (common, layout, feedback)
  config/       # Static configuration (navigation.ts)
  features/     # Feature-based organization (dashboard, search, etc.)
  hooks/        # Global hooks (useAuth)
  layouts/      # Application shell (AppLayout)
  providers/    # Context providers (AuthProvider, QueryProvider)
  services/     # Global services (token-storage)
  styles/       # CSS tokens and globals
`

### Architectural Decisions
- **Feature-Based Architecture**: All feature-specific pages are located inside src/features/<feature_name>/pages/.
- **Kebab-Case Naming**: All files follow kebab-case to ensure consistency across OS platforms.
- **API Layer**: Centralized piClient instance in src/api/client.ts. Interceptors are defined in interceptors.ts for clean token injection.
- **Authentication Infrastructure**: The AuthProvider handles checking 	oken-storage.ts for JWTs. ProtectedRoute wrapper secures the main layout shell.
- **Config-Driven Sidebar**: Sidebar items are rendered dynamically from src/config/navigation.ts, preparing the foundation for future RBAC features (disabling or hiding options based on roles).
- **Reusable Feedback Components**: Core feedback elements like <PageLoader>, <PageSkeleton>, <PageError>, <EmptyState>, and <ErrorBoundary> standardize loading and error states across the app.

### Conventions Introduced
- **No business logic in UI**: Components solely handle presentation.
- **Theme**: Moved away from generic styling to a strict, dark-first, dense layout utilizing styles/tokens.css and styles/globals.css.
