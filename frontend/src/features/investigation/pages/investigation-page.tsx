...
import { investigationApi, InvestigationIndicatorBundle } from '../api/investigation-api';
...
import { AlertHistoryPanel } from '../components/alert-history-panel';
import { Skeleton } from '@/components/ui/skeleton';

export const InvestigationPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [bundle, setBundle] = useState<InvestigationIndicatorBundle | null>(null);
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    setLoading(true);
    investigationApi.getIndicatorBundle(id).then(data => {
      if (isMounted) {
        setBundle(data);
        setLoading(false);
      }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 flex flex-col min-h-screen">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold tracking-tight">Indicator Investigation</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 flex flex-col gap-6">
          <EntitySummaryCard 
            title={bundle.indicator.value} 
            type={bundle.indicator.type}
            riskScore={bundle.risk_score} 
            tags={bundle.indicator.tags}
            attributes={bundle.indicator}
          />
          <AlertHistoryPanel indicatorId={id!} />
          <TimelinePanel timeline={bundle.timeline} />
        </div>
        
        <div className="md:col-span-2 flex flex-col gap-6">
          <div className="h-[500px] border rounded-lg overflow-hidden shadow-sm bg-background">
            <RelationshipExplorer entityId={id!} entityType="indicator" />
          </div>
